...
async fn seed_initial_data(pool: &DbPool, config: &AppConfig) -> Result<(), AppError> {
    ...
    if user_count == 0 {
        ...
        if let Some(user) = existing_user {
            ...
            info!("Updating existing single administrator email to '{}' and syncing password from environment...", clean_email);
            let new_hash = hash_password(clean_password)?;
            sqlx::query("UPDATE users SET email = ?, password_hash = ?, updated_at = CURRENT_TIMESTAMP")
                .bind(&clean_email)
                .bind(&new_hash)
                .execute(pool)
                .await
                .map_err(|e| AppError::Internal(format!("Failed to sync admin credentials: {}", e)))?;
            info!("Administrator credentials successfully synced with environment configuration.");
        } else {
            info!("Seeding administrator user for '{}' from environment configuration...", clean_email);
            let user_id = format!("usr_{}", uuid::Uuid::new_v4().simple());
            let password_hash = hash_password(clean_password)?;
            sqlx::query("INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)")
                ...
                .await
                .map_err(|e| AppError::Internal(format!("Failed to seed admin user: {}", e)))?;
            info!("Administrator created with email: {}", clean_email);
        }
    }
