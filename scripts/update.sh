...
if [ -f /etc/caddy/Caddyfile ] && [ -f "$ENV_ACTIVE" ]; then
    ...
    caddy_port=$(grep "^APP_PORT=" "$ENV_ACTIVE" | cut -d= -f2 || echo "5002")
    public_url=$(grep "^PUBLIC_BASE_URL=" "$ENV_ACTIVE" | cut -d= -f2 || echo "")
    caddy_domain=$(echo "$public_url" | sed -e 's|^[^/]*//||' -e 's|/.*$||')
